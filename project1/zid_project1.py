...
COLWIDTHS = dict(zip(COLUMNS,[14,11,19,18,6,20]))
print(COLWIDTHS)
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    file_name = f'{tic.lower()}_prc.dat'

    file_path = os.path.join(DATDIR,file_name)
    print(file_path)
...
def line_to_dict(line):
    ...
    for key,value in COLWIDTHS.items():
        print(value)          #值是对应的宽度
        count += value       #结束位置
        dct[key] = line[index:count]
        index += value      #开始的位置

    return dct
print(line_to_dict("00000000000000 2005-09-27018.19491004943847719.299999221.0490021.399999618530273"))
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        ...
        elif (all(item in list(tic_exchange_dic.keys()) for item in tickers_lst) is False):           #看最后一行是否在字典里（T为大写，改成小写就不会error了）
            raise Exception('tickers_lst) contains a ticker that does not correspond to a key tickers_lst)')
#tic_exchange_dic = {'tsm':'nyse','aal':'nasdag'}
#tickers_lst = ['aal','Tsm']
#print(verify_tickers(tic_exchange_dic,tickers_lst))
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        ...
        elif (all(item in COLUMNS for item in col_lst) is False):
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    if tickers_lst is None:
        tickers_lst_new = tic_exchange_dic.keys()
    else:
        tickers_lst_new = tickers_lst
    ...
    final_dict = {}
    for items in tickers_lst_new:
        inside_dict = {}

        exchange = tic_exchange_dic[items]
        inside_dict['exchange'] = exchange

        data_full = read_dat(items)

        data_list = [line_to_dict(item) for item in data_full]
        # print(data_list)
        data = []
        for tem in data_list:
            if col_lst is None:
                ...
                pair = dict((key, value) for (key, value) in tem.items() if key in col_lst)
                data.append(pair)

        inside_dict['data'] = data
        final_dict[items] = inside_dict
